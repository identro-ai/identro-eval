...
class YamlService {
    ...
    async generateAllAfterAnalysis(evalSpec) {
        try {
            const { YamlGenerator, VersionManager } = await Promise.resolve().then(() => __importStar(require('../_internal/core')));
    ...
    async updateAfterTestGeneration(evalSpec, enrichedEntities) {
        try {
            const { YamlGenerator } = await Promise.resolve().then(() => __importStar(require('../_internal/core')));
            const yamlGenerator = new YamlGenerator(this.projectPath);
